...
function getDesktopTitle(printess: iPrintessApi): HTMLElement {
  ...
  basketBtn.innerText = printess.gl("ui.basket");
...
function getValidationOverlay(printess: iPrintessApi, error: iExternalPropertyError) {
  ...
  title.textContent = printess.gl(`errors.${error.errorCode}Title`);
  ...
  ok.onclick = () => {
    ...
    printess.selectFrames(error.propertyId);
  }
...
function getDesktopStepsUi(printess: iPrintessApi): HTMLElement {
  ...
  if (printess.hasNextStep()) {
    ...
    nextStep.innerText = printess.gl("ui.basket");
